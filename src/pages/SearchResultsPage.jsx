...
const SearchResultsPage = () => {
  ...
  const formatReviewCount = (count) => {
    // Handle cases where count might be missing
          ...
          {vendors.map((vendor, index) => (
            ...
            >
              <div className="vendor-image-container">
                {/* Added a placeholder for missing logos */}
                <img src={vendor.logoUrl || 'https://via.placeholder.com/150'} alt={vendor.businessName} className="vendor-image" />
              </div>
              <div className="vendor-details">
                <h2 className="vendor-name">{index + 1}. {vendor.businessName}</h2>
                <div className="vendor-rating-info">
                  <StarRating rating={vendor.numericalRating} />
                  {/* Added a fallback for the numerical display */}
                  <span className="numerical-rating">{vendor.numericalRating || '0.0'}</span>
                ...
                <div className="vendor-meta">
                  ...
                  {vendor.status && (
                    <>
                      <span className="dot-separator">•</span>
                      <span className="status-text">{vendor.status}</span>
                    </>
                  )}
                </div>
                {/* Prevented error if description is missing */}
